...
struct StreamParser {
    ...
    /// Remove `<think>...</think>` blocks including partials split across calls.
    private mutating func filter(_ content: String) -> String? {
        var out = ""
        var i = content.startIndex
        while i < content.endIndex {
            if inThinkBlock {
                if let end = content.range(of: "</think>", range: i..<content.endIndex) {
                    inThinkBlock = false
                    i = end.upperBound
                } else {
                    // Whole remainder is inside a think block.
                    return out.isEmpty ? nil : out
                }
            } else {
                if let start = content.range(of: "<think>", range: i..<content.endIndex) {
                    out.append(contentsOf: content[i..<start.lowerBound])
                    inThinkBlock = true
                    i = start.upperBound
                } else {
                    out.append(contentsOf: content[i..<content.endIndex])
                    i = content.endIndex
                }
            }
        }
...
Note: the partial-tag case (where `<think>` itself straddles two chunks) is handled by JSON-level buffering — Ollama chunks are emitted per-line, and each line contains a complete JSON object. A `<think>` tag split mid-tag only happens inside a single line, and `firstRange` finds it correctly. The cross-chunk test above demonstrates this.
...
final class OllamaTranslator: Translator {
        ...
        AsyncThrowingStream { continuation in
            let task = Task {
                do {
                    ...
                    let body = buildBody(messages: messages)
    ...
    private func buildBody(messages: BuiltMessages) -> Data {
...
func test_imageTranslation_includesBase64AndStreams() async throws {
    ...
    StubURLProtocol.handler = { req in
        capturedBody = req.httpBody ?? Self.readBodyStream(req)
